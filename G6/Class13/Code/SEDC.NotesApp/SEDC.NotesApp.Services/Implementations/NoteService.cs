...
namespace SEDC.NotesApp.Services.Implementations
{
    public class NoteService : INoteService
    {
        ...
        private User ValidateNoteData(AddNoteDto addNoteDto)
        {
            ...
            if (userDb == null)
            {
                throw new DataException($"User with id {addNoteDto.UserId} does not exist");
            }

            //if (string.IsNullOrEmpty(addNoteDto.Text))
            //{
            //    throw new DataException("Text is required field");
            //}
            ////Text is not null or empty
            //if (addNoteDto.Text.Length > 100)
            //{
            //    throw new DataException("Text can not contain more than 100 characters");
            //}
